...
int MINAL_TIMEOUT_NA_OFERTY() {
    return ((clock() - timeout_start_time) * 1000 / CLOCKS_PER_SEC) > timeout_duration_ms;
}
...
// ----- funkcja obsługi odłożonych żądań -----
void ObsluzOdlozoneZadaniaDlaMiasta(int id_miasta_do_sprawdzenia) {
    ...
    for (int i = 0; i < liczba_odlozonych_REQ_dla_miasta[id_miasta_do_sprawdzenia]; ++i) {
        ...
        if (status_miasta[id_miasta_do_sprawdzenia] == WOLNE) {
            if ((status_miasta[id_miasta_do_sprawdzenia] != OCZEKUJE_NA_ACK_DLA_KONKRETNEGO || wybrane_miasto_do_REQ != id_miasta_do_sprawdzenia) ||
                (zegar_mojego_zadania_o_konkretne_miasto > odlozonego_req.zegar_zglaszajacego) ||
                (zegar_mojego_zadania_o_konkretne_miasto == odlozonego_req.zegar_zglaszajacego && moj_rank_mpi > odlozonego_req.id_zglaszajacego)) {
        ...
        if (czy_wyslac_ack_teraz) {
            ...
            WYPISZ_LOG(moj_rank_mpi, zegar_lamporta, "Wysyłam odłożony ACK do " + odlozonego_req.id_zglaszajacego + " dla miasta " + id_miasta_do_sprawdzenia);
        } else {
            NOWA_LISTA_ODLOZONYCH_REQ[nowa_liczba_odlozonych_REQ++] = odlozonego_req;
        }
    }
    memcpy(odlozone_REQ_dla_miasta[id_miasta_do_sprawdzenia], NOWA_LISTA_ODLOZONYCH_REQ, nowa_liczba_odlozonych_REQ * sizeof(ODLOZONE_REQ));
    ...
    while (1) {
        ...
        if (aktualna_faza_ubiegania_sie == CZEKAM_NA_OFERTY) {
            if (MINAL_TIMEOUT_NA_OFERTY() || OTRZYMANO_WYSTARCZAJACO_OFERT()) {
                ...
                for (int i = 0; i < liczba_otrzymanych_ofert; ++i) {
                    ...
                    if (status_miasta[oferta.id_oferowanego_miasta] != ODPOCZYWA || oferta.zegar_dostepnosci_miasta_z_oferty < miasto_odpoczywa_do_zegara[oferta.id_oferowanego_miasta]) { // [cite: 7]
                        ...
                        } else if (oferta.zegar_dostepnosci_miasta_z_oferty == najlepszy_zegar_dostepnosci && oferta.id_oferowanego_miasta < wybrane_miasto_do_REQ) { // [cite: 9]
                            wybrane_miasto_do_REQ = oferta.id_oferowanego_miasta;
                        }
                    }
                }
                ...
                if (wybrane_miasto_do_REQ != -1) {
                    zegar_lamporta++;
                    zegar_mojego_zadania_o_konkretne_miasto = zegar_lamporta;
                    licznik_otrzymanych_ACK_dla_konkretnego_miasta = 0;
                    WYPISZ_LOG(moj_rank_mpi, zegar_lamporta, "Wybrałem miasto " + wybrane_miasto_do_REQ + ". Wysyłam REQ_SPECIFIC_CITY.");
                    ROZESLIJ_DO_WSZYSTKICH_INNYCH(REQ_SPECIFIC_CITY, moj_rank_mpi, zegar_mojego_zadania_o_konkretne_miasto, wybrane_miasto_do_REQ);
        ...
        if (aktualna_faza_ubiegania_sie == WYBRALEM_OFERTE_CZEKAM_NA_ACK) {
            if (licznik_otrzymanych_ACK_dla_konkretnego_miasta == (N_ZABOJCOW - 1)) {
                zegar_lamporta++;
                WYPISZ_LOG(moj_rank_mpi, zegar_lamporta, "Otrzymałem wszystkie ACK. Wchodzę do miasta " + wybrane_miasto_do_REQ + ".");
                ...
                // opuszczanie miasta
                zegar_lamporta++;
                WYPISZ_LOG(moj_rank_mpi, zegar_lamporta, "Opuszczam miasto " + wybrane_miasto_do_REQ + ".");
        ...
        if (msg.typ != -1) {
            ...
            WYPISZ_LOG(moj_rank_mpi, zegar_lamporta, "Odebrałem " + msg.typ + " od " + msg.id_nadawcy + " z zegarem " + msg.zegar_nadawcy_oryginalny);
            ...
            switch (msg.typ) {
                ...
                case REQ_SPECIFIC_CITY: {
                    ...
                    int zegar_zadajacego_oryginalny = msg.zegar_nadawcy_oryginalny; // używamy oryginalnego zegara żądania do porównań

                    // logika Ricarta-Agrawali
                    int czy_odroczyc = 0;
                    if (status_miasta[id_zad_miasta] == W_MIESCIE && wybrane_miasto_do_REQ == id_zad_miasta) { // ja jestem w tym mieście
                        czy_odroczyc = 1;
                    } else if (status_miasta[id_zad_miasta] == OCZEKUJE_NA_ACK_DLA_KONKRETNEGO && wybrane_miasto_do_REQ == id_zad_miasta) { // ja się ubiegam
                        if (zegar_mojego_zadania_o_konkretne_miasto > zegar_zadajacego_oryginalny) {
                            czy_odroczyc = 1;
                        } else if (zegar_mojego_zadania_o_konkretne_miasto == zegar_zadajacego_oryginalny && moj_rank_mpi > msg.id_nadawcy) {
                            czy_odroczyc = 1;
                        }
                    } else if (status_miasta[id_zad_miasta] == ODPOCZYWA && zegar_lamporta < miasto_odpoczywa_do_zegara[id_zad_miasta]) { // Miasto odpoczywa i jeszcze nie jest dostępne [cite: 31]
                        czy_odroczyc = 1; // odroczymy, aż miasto przestanie odpoczywać
                    }

                    if (czy_odroczyc) {
                        DODAJ_DO_LISTY_ODLOZONEJ_REQ(id_zad_miasta, (ODLOZONE_REQ){msg.id_nadawcy, zegar_zadajacego_oryginalny});
                        WYPISZ_LOG(moj_rank_mpi, zegar_lamporta, "Odkładam REQ od " + msg.id_nadawcy + " o miasto " + id_zad_miasta);
                    } else {
                        zegar_lamporta++;
                        WYSLIJ_DO(msg.id_nadawcy, ACK_SPECIFIC_CITY, moj_rank_mpi, zegar_lamporta, id_zad_miasta);
                        WYPISZ_LOG(moj_rank_mpi, zegar_lamporta, "Wysyłam ACK do " + msg.id_nadawcy + " dla miasta " + id_zad_miasta);
                    }
                    break;
                }
                ...
                case RELEASE_CITY: {
                    ...
                    WYPISZ_LOG(moj_rank_mpi, zegar_lamporta, "Miasto " + id_zwol_miasta + " zwolnione przez " + msg.id_nadawcy + ", odpoczywa do " + zegar_kiedy_koniec_odpoczynku); // [cite: 36]
        ...
        for (int m = 0; m < M_MIAST; ++m) {
            if (status_miasta[m] == ODPOCZYWA && zegar_lamporta >= miasto_odpoczywa_do_zegara[m]) {
                status_miasta[m] = WOLNE;
                WYPISZ_LOG(moj_rank_mpi, zegar_lamporta, "Miasto " + m + " zakończyło odpoczynek.");
